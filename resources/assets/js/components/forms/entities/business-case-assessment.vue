<template>
  <el-tabs v-bind="$attrs" @tab-click="onTabClick">
    ...
    <el-tab-pane :data-name="item.assessed_process_form.replace('-', '_')" v-for="(item, index) in form.assessments" :key="index">
      ...
      <el-form-item-wrap
        :label="trans('forms.business_case_assessment.assessment_comments.label')"
        prop="assessment_comments"
        :required="item.process_form_decision_id === 2">
        <span slot="label-addons">
          <el-popover-wrap
            :description="trans('forms.business_case_assessment.assessment_comments.description')">
          </el-popover-wrap>
          <span class="instruction">
            {{ trans('forms.business_case_assessment.assessment_comments.instruction') }}
          </span>
        </span>
        <input-wrap
          ...
          :data-vv-as="trans('forms.business_case_assessment.assessment_comments.label')"
